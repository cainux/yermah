...
Yermah.prototype = {
  ...
  checkForNewMessages: function(messageId) {
    ...
    if(!this.includeOwnMessages) {
      this.getCurrentUser(function() { that.getMessages(messageId) });
    } else {
  ...
  getMessages: function(messageId) {
    ...
    var url = this.getApiCallUrl("messages/following");
    if(messageId > 0)
  ...
  notify: function(messages, references) {
    ...
    for(var i = 0, message; message = messages[i]; i++) {
      if(i < this.maxNotifications) {
        ...
        var user = this.getReference(message.sender_id, "user", references);
        var targetUrl = message.web_url;
        var urlType = "message";
        
        if(message.id != message.thread_id) {
          var thread = this.getReference(message.thread_id, "thread", references);
